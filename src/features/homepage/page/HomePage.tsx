    ...
    >
      ...
      <Flex flexDirection='column' gap='$20'>
        <SectionHeader title="Flash Sales" tag="Today's" leftExtra={<Timer/>}>
          <Flex flexDirection='row' justifyContent='space-between' alignItems='center' css={{minWidth:'100%'}}>
            
          <Flex gap='$4'>
          <ArrowButton  css={{transform:'rotate(180deg)'}}
            onClick={() =>
              setPage((p) => {
                // console.log('Prev clicked, current page:', p);
        ...
        <ButtonPrimary css={{ margin: '40px auto' }} onClick={()=>(navigate('/productsPage'))}>
          {' '}
          See All Products
        </ButtonPrimary>
      ...
      {/* Explore Our Products */}
        ...
        <Box css={{display:'flex', flexDirection:'column',
          ...
          <ButtonPrimary css={{maxWidth:"234px"}} onClick={()=>(navigate('/productsPage'))}>View All Products</ButtonPrimary>
        </Box>
      
      {/* New Arrivals */}
      <Box>
          <SectionHeader title="New Arrivals" tag="Featured">
          </SectionHeader>
      </Box>
      <Box>
        <NewArrivals/>
